window.addEvent("domready", function() {
	...
	$$('div.gallery').each(function(el,i){
		_fadeArray[i] = new fadeGallery($$("div.gallery")[0], {
			elementsParent: "ul.gallery-list",
			nextItem: ".btn-next",
			prevItem: ".btn-prev",
			speed: 400,
			paging: true,
		...
		if(this.visible < this.items.length) {
			...
			if(this.options.mode != "circle") {
				...
				if(this.options.paging) {
					this.paging = new Element("ul").injectInside(this.gallery.getElement(".paging-hold")).addClass("paging");
