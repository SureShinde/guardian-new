			...
			if ($customerAddressId){
                ...
                if ($this->getOnepage()->getQuote()->isVirtual()) {
					...
					foreach($cart_pid as $pid) {
						...
						if(($_product->getAttributeText('countries') == 'Canada' && $data['country_id'] != 'CA') || ($_product->getAttributeText('countries') == 'USA' && $data['country_id'] != 'US')) {
							array_push($products_removeids, $pid);
							array_push($productName,$_product->getName());
            ...
            if (!isset($result['error'])) {
				...
				elseif(preg_match('/\s*((P(OST)?.?\s*(O(FF(ICE)?)?)?.?\s+(B(IN|OX))?)|B(IN|OX))/i', implode(",", $data['street']))) {
						$result = array(
									'error' => 1,
									'goto_section' => 'shipping',
									'message' => Mage::helper('checkout')->__('We do not ship to PO Boxes.')
									);
					}
				else {
					$result['goto_section'] = 'shipping_method';
